...
model NuScaleModule_v4
  ...
  TRANSFORM.Fluid.Machines.Pump_SimpleMassFlow pump(redeclare package Medium =
        Modelica.Media.Water.StandardWater,
    use_input=true,                         m_flow_nominal=data.m_flow_steam)
  ...
  Modelica.Blocks.Sources.RealExpression realExpression(y=core.Q_total.y)
    annotation (Placement(transformation(extent={{234,-44},{254,-24}})));
  Modelica.Blocks.Interfaces.RealOutput Q_Total
    annotation (Placement(transformation(extent={{280,-44},{300,-24}})));
  Modelica.Blocks.Sources.RealExpression realExpression2(y=Steam_exit_enthalpy.h_out)
    annotation (Placement(transformation(extent={{236,8},{256,28}})));
  Modelica.Blocks.Interfaces.RealOutput Core_Tavg annotation (Placement(
        transformation(extent={{282,36},{300,54}}),     iconTransformation(
          extent={{282,36},{300,54}})));
  Modelica.Blocks.Interfaces.RealOutput SG_outlet_Enthalpy
    annotation (Placement(transformation(extent={{280,8},{300,28}})));
  Modelica.Blocks.Sources.RealExpression realExpression1(y=AvgCore_T.y)
    annotation (Placement(transformation(extent={{236,36},{256,56}})));
  ...
  TRANSFORM.Fluid.Interfaces.FluidPort_Flow port_a(redeclare package Medium =
        Modelica.Media.Water.StandardWater)
    annotation (Placement(transformation(extent={{122,18},{142,38}})));
  TRANSFORM.Fluid.Interfaces.FluidPort_State port_b(redeclare package Medium =
        Modelica.Media.Water.StandardWater)
    annotation (Placement(transformation(extent={{126,-106},{146,-86}})));
  Modelica.Blocks.Sources.RealExpression realExpression3(y=AvgCore_T.y)
    annotation (Placement(transformation(extent={{238,-18},{258,2}})));
  Modelica.Blocks.Interfaces.RealOutput SG_level annotation (Placement(
        transformation(extent={{280,-18},{298,0}}), iconTransformation(extent={
            {-224,-68},{-206,-50}})));
  Modelica.Blocks.Sources.RealExpression realExpression4(y=
        Secondary_Side_Pressure.p)
    annotation (Placement(transformation(extent={{236,-72},{256,-52}})));
  Modelica.Blocks.Interfaces.RealOutput SG_pressure
    annotation (Placement(transformation(extent={{282,-72},{302,-52}})));
equation
  ...
  connect(realExpression.y, Q_Total)
    annotation (Line(points={{255,-34},{290,-34}}, color={0,0,127}));
  connect(realExpression2.y, SG_outlet_Enthalpy)
    annotation (Line(points={{257,18},{290,18}}, color={0,0,127}));
  ...
  connect(realExpression1.y, Core_Tavg) annotation (Line(points={{257,46},{270,
          46},{270,45},{291,45}},           color={0,0,127}));
  ...
  connect(port_b, pump.port_a) annotation (Line(points={{136,-96},{114,-96},{
          114,-72},{92,-72}}, color={0,127,255}));
  connect(port_a, STHX.port_b_tube) annotation (Line(points={{132,28},{86,28},{
          86,14},{39,14}}, color={0,127,255}));
  connect(realExpression3.y, SG_level) annotation (Line(points={{259,-8},{282,
          -8},{282,-9},{289,-9}}, color={0,0,127}));
  connect(realExpression4.y, SG_pressure)
    annotation (Line(points={{257,-62},{292,-62}}, color={0,0,127}));
